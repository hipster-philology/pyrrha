...
class TestTokenCorrectWauchierCorpus(TokenCorrectHelpers):
    ...
    def test_edit_token(self):
        ...
        self.page.reload()
        token, row = self.edith_nth_row_value("un", corpus_id=self.CORPUS_ID)
        assert token.lemma == "un", "Lemma should have been changed"
        self.assert_saved(row)
        assert "at-row--changed" in row.get_attribute("class")
        self.page.reload()
    ...
    def test_edit_token_lemma_with_allowed_values(self):
        ...
        self.page.reload()
        token, row = self.edith_nth_row_value("un", id_row="1")
        self.assert_token_has_values(token, lemma="un")
        self.assert_saved(row)

        self.page.reload()
        self.page.wait_for_load_state("networkidle")
        token, row = self.edith_nth_row_value("WRONG", id_row="2")
        self.assert_token_has_values(token, lemma="saint")
        self.assert_invalid_value(row, "lemma")

        self.page.reload()
        token, row = self.edith_nth_row_value("ADJqua", value_type="POS", id_row="3")
    ...
    def test_edit_token_lemma_with_allowed_values_autocomplete(self):
        ...
        token, row = self.edith_nth_row_value(
            "d",
            id_row="1",
            autocomplete_selector=".anno-dropdown [data-val='devoir']",
        )
    ...
    def test_edit_POS(self):
        self.addCorpus(with_token=True, with_allowed_lemma=True, tokens_up_to=24)
        self.page.reload()
        token, row = self.edith_nth_row_value("ADJqua", id_row="1", value_type="POS")
    ...
    def test_edit_POS_autoselect(self):
        ...
        token, row = self.edith_nth_row_value(
            ...
            autocomplete_selector=".anno-dropdown [data-val='ADJqua']"
        )
        self.assert_token_has_values(token, lemma="de", POS="ADJqua")
        self.assert_saved(row)
    ...
    def test_edit_morph(self):
        self.addCorpus(with_token=True, with_allowed_lemma=True, tokens_up_to=24)
        self.page.reload()
        token, row = self.edith_nth_row_value("_", id_row="1", value_type="morph")
        self.assert_saved(row)
        self.assert_token_has_values(token, lemma="de", POS="PRE", morph="_")

        token, row = self.edith_nth_row_value("Not Allowed", id_row="2", value_type="morph")
        self.assert_saved(row)
    ...
    def test_edit_morph_with_allowed(self):
        ...
        token, row = self.edith_nth_row_value(
            ...
            corpus_id="1",
            value_type="morph",
            autocomplete_selector=".anno-dropdown [data-val='NOMB.=s|GENRE=m|CAS=n']",
        )
        self.assert_saved(row)
    ...
    def test_edit_token_with_filter(self):
        ...
        token, row = self.edith_nth_row_value("]", value_type="lemma", id_row="1")
        self.assert_saved(row)
...
class TestTokensCorrectFloovant(TokenCorrectHelpers):
    ...
    def test_edit_token_lemma_with_allowed_values_lemma_pos(self):
        ...
        self.page.reload()
        token, row = self.edith_nth_row_value("estoire1", id_row="1")
        self.assert_invalid_value(row, "POS")
        # Token are not updated because their POS value is invalid
    ...
    def test_edit_token_morph_with_allowed_values_lemma(self):
        self.addCorpus(with_token=True, with_allowed_lemma=True)
        self.page.reload()
        token, row = self.edith_nth_row_value("SomeMorph", id_row="1", value_type="morph")
        self.assert_saved(row)
    ...
    def test_edit_token_with_same_value(self):
        self.addCorpus(with_token=True)
        self.page.reload()
        token, row = self.edith_nth_row_value("seignor", id_row="1", value_type="lemma")
...
class TestTokensEditTwoCorpora(TokenCorrect2CorporaHelpers):
    ...
    def test_edit_token_lemma_with_allowed_values_lemma_pos(self):
        self.addCorpus(with_token=True, with_allowed_lemma=True, with_allowed_pos=True, tokens_up_to=24)
        token, row = self.edith_nth_row_value("saint", id_row="1")
        self.assert_saved(row)
        self.assert_token_has_values(token, lemma="saint")

        self.page.reload()
        token, row = self.edith_nth_row_value("seignor", id_row="2")
        self.assert_invalid_value(row, "lemma")
        self.assert_token_has_values(token, lemma="saint")
    ...
    def test_edit_token_lemma_with_typeahead_click(self):
        ...
        token, row = self.edith_nth_row_value(
            ...
            id_row="1",
            corpus_id="1",
            autocomplete_selector=".anno-dropdown [data-val='saint']",
        )
        self.assert_saved(row)
        self.assert_token_has_values(token, lemma="saint")
        ...
        token, row = self.edith_nth_row_value(
            ...
            corpus_id="2",
            autocomplete_selector=".anno-dropdown [data-val='seignor']",
        )
        self.assert_saved(row)
        self.assert_token_has_values(token, lemma="seignor")
    ...
    def test_correct_delete(self):
        ...
        row = self.page.locator(".at-row[data-token-order='1']").first
        assert row.locator(".at-cell.at-cell--form").first.text_content() == "seint", "Token was removed"
